...
exports.createHistory = async (req, res, next) => {
  try {
    ...
    //delete QRcode when the transaction is completed
...
exports.getAllHistory = (req, res, next) => {
  ...
  //create match object from querys data for $match in aggregate below
  ...
  History.aggregate([
    //create fields day, month & year from fild date then filter the result with $match
    {
      '$addFields': {
        'day': {
          '$dayOfMonth': '$date'
  ...
  .then((historyInfo) => {res.status(200).json(historyInfo)})
...
exports.getAllHistoryForOneUser = async (req, res, next) => {
  try {
    ...
    if ((req.params.param == "userId" && req.params.id == userId) || user.status == "admin" || (req.params.param == "partnerId" && req.params.id == partner._id)) {
      ...
      // create object filter from req.params for aggregate below
